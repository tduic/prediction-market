    ...
    async def get_overview() -> Dict[str, Any]:
        ...
        try:
            ...
            try:
                ...
                _net_pnl = float(_dl_row[0]) if _dl_row else 0.0
                _daily_loss = max(0.0, -_net_pnl)
                cfg_rc = get_config().risk_controls
                _daily_loss_limit = cfg_rc.starting_capital * cfg_rc.max_daily_loss_pct
                if _daily_loss_limit > 0:
                    daily_loss_pct_used = round(
                        _daily_loss / _daily_loss_limit * 100, 1
                    )
